...
## Автоматический аудит портала

**Claude может самостоятельно тестировать портал** через Playwright (headless browser):
- Скрипты аудита: `scripts/audit-portal.mjs` (быстрый), `scripts/audit-realworld.mjs` (полный CRUD)
- Скриншоты: `audit-screenshots/`
- Уведомления: Telegram бот @Stankoff_BPMS_Test_Bot (токен и chat_id на препроде в `/opt/stankoff-portal/.env`)

### Правила аудита

1. **Проверять ВСЁ** — создание, просмотр, редактирование, удаление заявок; связи; бизнес-процессы; чат; админку
2. **Как реальный пользователь** — клики, ввод данных, модалки, переходы (не только API)
3. **Не коммитить/пушить без одобрения пользователя** — все изменения только локально, показывать diff перед коммитом
4. **Отправлять статусы в Telegram** — о найденных багах, фиксах, прогрессе
5. **Актуализировать документацию** — при изменениях обновлять CLAUDE.md, docs/ARCHITECTURE.md, docs/API.md
6. **Использовать актуальные инструменты** — Playwright (свежая версия), современные паттерны
