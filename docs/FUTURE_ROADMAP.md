...
**Статус:** Часть пунктов реализована (AI, Legacy), остальное отложено
...
### 2.1 Реализовано ✅ (RAG по legacy заявкам)

Индексация legacy заявок (QD_requests + QD_answers) в pgvector:
- [x] AiModule с OpenAI/Ollama embeddings
- [x] RAG индексатор для legacy закрытых заявок с ответами
- [x] Семантический поиск через pgvector (HNSW index)
- [x] API: `POST /api/ai/search`, `GET /api/ai/indexer/stats`
- [x] Ссылки на legacy систему в результатах

### 2.2 Будущее: Документы оборудования (не реализовано)
...
1. **Высокий:** Production инфраструктура, backup, мониторинг (Sentry)
2. **Средний:** Фаза 3 (Wiki, Отчёты, Уведомления)
3. **Низкий:** Клиентский портал, другие отделы, индексация документов оборудования
