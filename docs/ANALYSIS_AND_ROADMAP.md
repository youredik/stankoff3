...
| Модуль | Статус | Описание |
|--------|--------|----------|
| **RecommendationsModule** | ✅ Базовый | Статистические рекомендации (не LLM) |
...
|--------|-------------------|--------------|
| **ML/AI** | Только статистические методы | Добавить LLM для интеллектуальной помощи |
| **Интеграция с legacy** | Не реализована | Нужен модуль синхронизации |
...
| **Логирование** | Базовое (консоль) | Централизованный сбор логов |
...
| **Интеграция с legacy CRM** | ❌ Нет | Требуется разработка |
| **Номенклатура товаров** | ❌ Нет | Требуется модуль Products |
| **Клиенты/Контрагенты** | ❌ Нет | Требуется модуль CRM |
| **Интеллектуальный ИИ** | ⚠️ Базовый | Есть статистика, нет LLM |
...
### Фаза 1: Интеграция с Legacy CRM (1-2 месяца)
...
- [ ] Создать read-only пользователя в legacy БД
- [ ] Настроить второй DataSource в NestJS
- [ ] Создать entities для ключевых таблиц
- [ ] Реализовать поиск клиентов/товаров
- [ ] Добавить UI-пикеры на frontend
- [ ] Настроить кэширование (Redis или in-memory)
- [ ] Протестировать на preprod с реальными данными

### Фаза 2: ИИ-ассистент и LLM интеграция (1-2 месяца)
...
| 3 | RAG: индексация документов, поиск похожих |
| 4 | UI компоненты: кнопка генерации ответа, похожие заявки |
| 5 | AI Service Tasks для BPMN |
| 6 | Тестирование, настройка промптов, оптимизация |
| 7 | Мониторинг расходов, fallback провайдеры |
...
1. **Интеграция с legacy CRM** — без этого невозможно полноценно работать с клиентами и товарами
2. **Настройка backup** — защита от потери данных
3. **Мониторинг и алерты** — видимость проблем

### Высокий приоритет

4. **ИИ-классификация заявок** — уменьшит ручную работу
5. **RAG для поиска решений** — ускорит обработку типовых проблем
6. **Готовые процессы для Сервиса** — быстрый запуск отделов
