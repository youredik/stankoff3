...
- Логи: `docker service logs stankoff-preprod_backup`
- Docker образ: `ghcr.io/youredik/stankoff3/backup:preprod` (собирается в CI/CD)
...
### Кэширование AI (сокращение расхода токенов)

При открытии заявки автоматически запускаются несколько AI-компонентов. Многоуровневое кэширование предотвращает повторные LLM-вызовы при повторном открытии.

**Backend кэши (in-memory Map, TTL + FIFO eviction):**

| Сервис | Ключ кэша | TTL | Макс. записей | Описание |
|--------|-----------|-----|---------------|----------|
| `AiAssistantService` (assistance) | `entityId` | 5 мин | 200 | Подсказки: похожие случаи, эксперты, рекомендации |
| `AiAssistantService` (sentiment) | `entityId:commentId` | 5 мин | 200 | Анализ настроения последнего комментария |
| `KnowledgeGraphService` | `entityId` | 5 мин | 200 | Граф связей заявки |
| `KnowledgeBaseService` (embeddings) | текст запроса | 5 мин | 200 | Embedding-вектор для идентичных запросов |
| `ClassifierService` (classifyAndSave) | entityId (БД) | 30 мин | — | Повторная классификация < 30 мин возвращает существующую из БД |
| `AiAssistantService` (summary) | `entityId` (БД) | 5 мин | — | Резюме переписки (>= 3 комментариев) |

**Особенности кэширования sentiment:**
- Ключ включает `commentId` последнего комментария
- При появлении нового комментария ключ автоматически меняется → кэш невалиден
- `invalidateCache(entityId)` очищает все sentiment-записи для entity (по префиксу `entityId:`)

**Frontend кэши (Zustand store `useAiStore`):**

| Данные | TTL | Инвалидация | Дедупликация |
|--------|-----|-------------|--------------|
| Assistance (подсказки) | 5 мин | `invalidateAssistance(entityId)` | По `assistanceLoading` Map |
| Classification | 10 мин | `onClassificationReady(entityId)` + WebSocket | По `classificationLoading` Map |
| Summary (резюме) | 5 мин | При изменении `commentCount` | По `summaryLoading` Map |
| Knowledge Graph | 5 мин | `invalidateKnowledgeGraph(entityId)`, `forceRefresh` | По `knowledgeGraphLoading` Map |

**Логирование использования (`AiUsageLog`):**
- Все LLM-операции логируются: `classify`, `generate`, `sentiment`, `summary`
