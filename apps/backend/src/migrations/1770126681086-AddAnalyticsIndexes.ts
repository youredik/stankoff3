...
export class AddAnalyticsIndexes1770126681086 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`CREATE INDEX "idx_entities_workspace_status" ON "entities" ("workspaceId", "status")`);
        await queryRunner.query(`CREATE INDEX "idx_entities_workspace_created" ON "entities" ("workspaceId", "createdAt" DESC)`);
        await queryRunner.query(`CREATE INDEX "idx_entities_workspace_assignee" ON "entities" ("workspaceId", "assigneeId")`);
        await queryRunner.query(`CREATE INDEX "idx_entities_assignee" ON "entities" ("assigneeId") WHERE "assigneeId" IS NOT NULL`);

        // B-tree индексы для comments
        await queryRunner.query(`CREATE INDEX "idx_comments_entity_created" ON "comments" ("entityId", "createdAt")`);
        ...
        await queryRunner.query(`CREATE INDEX "idx_entities_data_gin" ON "entities" USING GIN (data jsonb_path_ops)`);
        await queryRunner.query(`CREATE INDEX "idx_entities_linked_gin" ON "entities" USING GIN ("linkedEntityIds")`);
        await queryRunner.query(`CREATE INDEX "idx_comments_mentions_gin" ON "comments" USING GIN ("mentionedUserIds")`);
        await queryRunner.query(`CREATE INDEX "idx_audit_details_gin" ON "audit_logs" USING GIN (details jsonb_path_ops)`);
    }
